...
class ApiPartnerProfileSyncAdapterTest {
    ...
    void returnsExternalUserIdFromLenderResponse() throws Exception {
        ...
        when(httpClient.postEnvelopeWithInteraction(anyString(), anyString(), anyString(), anyString()))
                .thenReturn(new ApiPartnerHttpClient.EnvelopeResult(envelope, 1L));

        ApiPartnerProfileSyncAdapter adapter = new ApiPartnerProfileSyncAdapter(httpClient, objectMapper);
    ...
    void buildsLenderUpsertBodyWithMobileNoProfileAndDevice() throws Exception {
        ...
        verify(httpClient).postEnvelopeWithInteraction(
                eq(ApiPartnerProfileSyncAdapter.UPSERT_PATH),
                bodyCaptor.capture(),
                eq(ApiPartnerProfileSyncAdapter.BUSINESS_TYPE),
                eq("OPEN_USER_1")
        );
