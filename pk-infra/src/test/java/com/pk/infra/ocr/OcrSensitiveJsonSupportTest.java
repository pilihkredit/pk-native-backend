...
class OcrSensitiveJsonSupportTest {
    ...
    void encryptsIdCardAndMotherNameInPlaceAndStoresImageAsRef() throws Exception {
        ...
        when(imageStore.store(eq(9L), eq(BiometricImageKind.FACE), any()))
                .thenReturn("enc://face/9");
        ...
        String input = """
                ...
                """;
        String sanitized = support.sanitizeForStorage(input, 9L);
        ...
        assertThat(root.path("face_photo_image").path("encryptedRef").asText()).isEqualTo("enc://face/9");
    }
}
