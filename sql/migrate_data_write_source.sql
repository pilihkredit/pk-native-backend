-- Add write-source column on shared writable tables (APP / JOB / CALLBACK).
...
SET @sql = IF(
    ...
    'SELECT ''skip: credit_lender_status_query.source'' AS migration_info'
);
...
SET @sql = IF(
    ...
    'ALTER TABLE external_interaction
        ADD COLUMN source VARCHAR(32) NULL COMMENT ''Write source: APP / JOB / CALLBACK''
        AFTER duration_ms',
    'SELECT ''skip: external_interaction.source'' AS migration_info'
);
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
